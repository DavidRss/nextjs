...
const Navigate = () => {
  ...
  return (
    <section
      className={`w-full  flex items-center justify-center sticky navbar  z-50 ${
        isSticky ? "shadow-md bg-secBg -top-0.5" : ""
      }`}
    >
      {/*  */}
      <div
        className={`w-full max-w-8xl px-16 hidden md:flex items-center shadow-lg rounded-xl relative py-2 ${
          isSticky
            ? "px-0 shadow-none -top-0 w-full bg-secBg"
            : " bg-container "
        }`}
      >
        {/* -top-16 */}
        <div className="flex w-full h-14 xl:h-24 gap-14 justify-center xl:justify-between">
          <div
            className={`relative -top-10 hidden xl:block ${
              ...
              <div className="text-white w-full h-full">
                <ul className="flex items-center w-full justify-between gap-5 h-full flex-wrap">
                  <Link to="/" className="h-full">
                    <li
                      className={`flex nav__item hover:border-b-8 border-main rounded-b items-center lg:text-2xl gap-3 h-full text-white hover:text-main font-semibold transition-all cursor-pointer ${isActive(
                        "/reward"
                      )} h-full  sm:text-center`}
                    >
        ...
        <button
          type="button"
          className="flex absolute items-center bottom-32 right-16 justify-start py-3 text-white bg-white bg-opacity-20 pl-3 pr-5 text-2xl font-semibold border-4 border-white rounded-full gap-5 transition-all hover:scale-105 hover:shadow-lg hover:shadow-main/50"
        ...
        >
          ...
          <span className={`${hovered ? "-translate-y-1" : ""} transition-all`}>
            Regarder la Vidéo
          </span>
        </button>
      </div>
