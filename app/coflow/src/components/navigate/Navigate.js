...
import avatarMain from '../../assets/1202/channels4_profile.jpg';
...
const Navigate = () => {
    ...
    const isActive = (path) => {
        return location.pathname === path ? 'border-b-4 border-main text-main' : 'text-text';
    };
    ...
    return (
        <>
            <section className='w-full px-4 flex mdMax:hidden flex-col gap-6 items-center z-50 relative -top-3'>
                <img src={avatarMain} alt="avatar" className='max-w-32 rounded-full ring-4 ring-gray-200' />
                <div className='flex flex-col items-center gap-5'>
                    <h2 className='text-3xl font-semibold' style={{letterSpacing: '-1.28px'}}>{name}</h2>
                    <span className='text-lg font-semibold'>{subcribers.toLocaleString()} Subcribers</span>
                </div>

            </section>
            <section className={`bg-thrBg w-full h-full flex items-center justify-center sticky navbar pt-0 z-60 ${isSticky ? 'shadow-md' : ''}`}>
                <Container>
                                ...
                                <nav className='h-full'>
                                    <ul className='flex items-center w-full justify-center gap-5 xl:gap-10 h-full'>
                                        {links.map((link, index) => (
                                            ...
                                            key={index}
                                            >
                                            <li className={`hover:text-main flex gap-3 justify-center items-center font-semibold transition-all cursor-pointer hover:border-b-4 hover:border-main ${isActive(link.to)} h-full sm:w-28 sm:text-center`}>
                                                <img src={link.icon} className={`${hoveredIndex === index ? '-translate-y-0.5' : ''} hidden sm:block w-6 transition-all duration-80 brightness-0`} alt={link.label} />
                                                <span className={`${hoveredIndex === index ? '-translate-x-0.5' : ''} transition-all duration-80`}>{link.label}</span>
                                            </li>
                                            </Link>
                                        ))}
                                    </ul>
