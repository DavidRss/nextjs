...
const Navigate = () => {
    ...
    const isActive = (path) => {
        return location.pathname === path ? 'border-b-2 border-primary text-primary' : '';
    };
    ...
    return (
        <section className={`bg-secondary w-full py-5 flex items-center justify-center sticky navbar -top-0.5 z-50 ${isSticky ? 'shadow-md' : ''}`}>
            <Container>
                            ...
                            <nav className='h-full'>
                                <ul className='flex items-center w-full justify-center gap-5 xl:gap-10 h-full'>
                                    ...
                                    <Link to="/chat" className='h-full'>
                                        <li className={`text-gray-900 hover:text-primary font-semibold transition-all cursor-pointer hover:border-b-2 hover:border-primary ${isActive('/chat')} h-full pt-2 xl:pt-9 sm:w-28 sm:text-center`}>Chat</li>
                                    </Link>
                                    <Link to="/reward" className='h-full'>
                                        <li className={`text-gray-900 hover:text-primary font-semibold transition-all cursor-pointer hover:border-b-2 hover:border-primary ${isActive('/reward')} h-full pt-2 xl:pt-9 sm:w-28 sm:text-center`}>Reward</li>
                                    </Link>
                                </ul>
                            </nav>
                        </div>
                        <button className="btn btn-primary px-8 py-4 text-white hidden xl:block">Se connecter</button>
                    </div>
                </div>            
           </Container>
        </section>
