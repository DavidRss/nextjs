                                    ...
                                    {levelsData.map((lvl, index) => (
                                        <li key={index} className={`cup step step-neutral w-full ml-5 sm:ml-10 ${spending >= lvl.spend ? 'step-primary' : ''}`}>
                                            <div className='flex flex-col items-start  w-full max-w-xl'>
                                                <span className='font-normal text-gray-900 text-sm opacity-50'>Level {lvl.level}</span>
                                                <h1 className='font-semibold text-gray-900 text-xl'>{lvl.title}</h1>  

                                                {lvl.spend !== 0 && (
                                                        <span className='font-normal text-gray-900 text-sm opacity-50'>After spending {lvl.spend.toLocaleString()} USD</span>
                                                    )}
                                                    {lvl.spend !== 0 && (
                                                        <div className="collapse collapse-arrow w-52 sm:w-full border-pagBg bg-white rounded-lg mt-4 mb-4 sm:mb-0">
                                                        <input type="radio" name="my-accordion-2" />
                                                        <div className="collapse-title">
                                                            <h1 className='font-semibold text-gray-900 text-sm sm:text-xl text-left'>Benefits</h1>
                                                        </div>
                                                        <div className="collapse-content text-left px-4">
                                                            <span className='font-normal text-gray-900 text-xs sm:text-sm opacity-50'>{lvl.benefits.desc}</span>
